...
void Calculate_electronicGroundState(SPARC_OBJ *pSPARC) {
    ...
    //calculate desctiptors at the end of SCF cycle
    if (pSPARC->CalcMCSHFlag == 1){
        if (strcmpi(pSPARC->XC,"GGA_PBE") == 0){
            ...
            #ifdef DEBUG
            if (rank == 0) printf("Calculate multipole descriptors \n");
            #endif
            t1 = MPI_Wtime();
            Calculate_MCSHDescriptors(pSPARC, &mp, iter);
            t2 = MPI_Wtime();
            #ifdef DEBUG
            if (rank == 0) printf("Time for calculating multipole descriptors: %.3f ms\n", (t2-t1)*1e3);
            #endif
        }
    }
...
void scf_loop(SPARC_OBJ *pSPARC) {
    ...
    while (SCFcount < pSPARC->MAXIT_SCF) {
        ...
        // before checking the stopping criterion, check if MCSH flag == 2 and calculate descriptors after every iteration
        if (pSPARC->CalcMCSHFlag == 2){
            if (strcmpi(pSPARC->XC,"GGA_PBE") == 0){
                MULTIPOLE_OBJ mp;
                Multipole_Initialize(pSPARC, &mp);
                Calculate_MCSHDescriptors(pSPARC, &mp, SCFcount);
            }
        }
