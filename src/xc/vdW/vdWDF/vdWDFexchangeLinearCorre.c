...
void Calculate_Vxc_vdWExchangeLinearCorre(SPARC_OBJ *pSPARC, XCCST_OBJ *xc_cst, double *rho) { 
    ...
    double *vdWDFVx1 = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(vdWDFVx1 != NULL); // d(n\epsilon_x)/dn in dmcomm_phi
    // It has two Nd_d: d(E_x)/d(n_up) and d(E_x)/d(n_dn)
    double *vdWDFVx2 = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(vdWDFVx2 != NULL); // d(n\epsilon_x)/d|grad n| / |grad n| in dmcomm_phi
    ...
    Drho_x = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(Drho_x != NULL); // in vdWDF we only need the grad of n_up and n_dn. 
    Drho_y = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(Drho_y != NULL);
    Drho_z = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(Drho_z != NULL);
    DDrho_x = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(DDrho_x != NULL);
    DDrho_y = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(DDrho_y != NULL);
    DDrho_z = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(DDrho_z != NULL);
    sigma = (double *) malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(sigma != NULL);
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, rho + (pSPARC->Nspden - 1) * DMnd, pSPARC->Drho[0], 0, pSPARC->dmcomm_phi);
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, rho + (pSPARC->Nspden - 1) * DMnd, pSPARC->Drho[1], 1, pSPARC->dmcomm_phi);
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, rho + (pSPARC->Nspden - 1) * DMnd, pSPARC->Drho[2], 2, pSPARC->dmcomm_phi);
    memcpy(Drho_x, pSPARC->Drho[0], DMnd * pSPARC->Nspden * sizeof(double));
    memcpy(Drho_y, pSPARC->Drho[1], DMnd * pSPARC->Nspden * sizeof(double));
    memcpy(Drho_z, pSPARC->Drho[2], DMnd * pSPARC->Nspden * sizeof(double));
    ...
    if(pSPARC->cell_typ > 10 && pSPARC->cell_typ < 20){
        ...
        for(i = 0; i < DMnd * pSPARC->Nspden; i++){
    ...
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, Drho_x, DDrho_x, 0, pSPARC->dmcomm_phi);
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, Drho_y, DDrho_y, 1, pSPARC->dmcomm_phi);
    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices, pSPARC->Nspden, 0.0, Drho_z, DDrho_z, 2, pSPARC->dmcomm_phi);
