...
void Setup_Comms(SPARC_OBJ *pSPARC) {
    ...
    if (pSPARC->PrintEnergyDensFlag == 1) {
        ...
        pSPARC->KineticRho = (double *) calloc(pSPARC->Nd_d_dmcomm * ncopy, sizeof(double));
        assert(pSPARC->KineticRho != NULL);
        if (pSPARC->usefock > 0) {
            pSPARC->ExxRho = (double *) calloc(pSPARC->Nd_d_dmcomm * ncopy, sizeof(double));
            pSPARC->ExxRho_approx = (double *) calloc(pSPARC->Nd_d_dmcomm * ncopy, sizeof(double));
            assert(pSPARC->ExxRho != NULL && pSPARC->ExxRho_approx != NULL);
        }
    }
