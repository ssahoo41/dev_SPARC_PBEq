                         ...
                         double *Hp, double *Mp, int k, int spn_i, MPI_Comm comm) 
{
    ...
    if (pSPARC->npband > 1) { 
        if (pSPARC->CyclixFlag == 1) {
            ...
            pdsyrk_("U", "T", &Ns, &Nd_blacscomm, &alpha, pSPARC->Yorb_BLCYC, &ONE, &ONE, 
        ...
        if (pSPARC->CyclixFlag == 1) {
            ...
            #endif   
            cblas_dsyrk(CblasColMajor, CblasUpper, CblasTrans, Ns, Nd_blacscomm, alpha, 
                pSPARC->Yorb_BLCYC, Nd_blacscomm, beta, Mp, Ns);
        }
    }
