...
void Calculate_nonlocal_kinetic_stress_cyclix(SPARC_OBJ *pSPARC)
{
    ...
    if (pSPARC->npspin > 1) {    
        if (pSPARC->spincomm_index == 0){
            MPI_Reduce(MPI_IN_PLACE, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->spin_bridge_comm);
            MPI_Reduce(MPI_IN_PLACE, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->spin_bridge_comm);
        } else{
            MPI_Reduce(&stress_nl, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->spin_bridge_comm);
            MPI_Reduce(&stress_k, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->spin_bridge_comm);
        }
    }
    ...
    if (pSPARC->npband > 1) {
        if (pSPARC->bandcomm_index == 0){
            MPI_Reduce(MPI_IN_PLACE, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->blacscomm);
            MPI_Reduce(MPI_IN_PLACE, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->blacscomm);
        } else{
            MPI_Reduce(&stress_nl, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->blacscomm);
            MPI_Reduce(&stress_k, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->blacscomm);
        }
    }
...
{ 
    ...
    if (pSPARC->npkpt > 1) {    
        if (pSPARC->kptcomm_index == 0){
            MPI_Reduce(MPI_IN_PLACE, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->kpt_bridge_comm);
            MPI_Reduce(MPI_IN_PLACE, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->kpt_bridge_comm);
        } else{
            MPI_Reduce(&stress_nl, &stress_nl, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->kpt_bridge_comm);
            MPI_Reduce(&stress_k, &stress_k, 1, MPI_DOUBLE, MPI_SUM, 0, pSPARC->kpt_bridge_comm);
        }
    }
