        ...
        for (iat = 0; iat < Atom_Influence_nloc[ityp].n_atom; iat++) {
            ...
            if (pSPARC->CyclixFlag) {
                ...
                for (ipos = 0; ipos < ndc; ipos++) {
                    ...
                    yy = y - yi;
                    zz = z - zi;
                    rc_pos_x[ipos] = pSPARC->RotM_cyclix[0] * xx + pSPARC->RotM_cyclix[1] * yy;
                    rc_pos_y[ipos] = pSPARC->RotM_cyclix[3] * xx + pSPARC->RotM_cyclix[4] * yy;
                    rc_pos_z[ipos] = zz;
                    ...
                    for (m = -l; m <= l; m++) {
                        ...
                        if (pSPARC->CyclixFlag) {
                            for (ipos = 0; ipos < ndc; ipos++) {
                                ...
                                // indx = Atom_Influence_nloc[ityp].grid_pos[iat][Ind_sort[ipos]];
                                // (*nlocProj)[ityp].Chi_cyclix[iat][col_count*ndc+Ind_sort[ipos]] = Ylm[Ind_sort[ipos]] * UdV_sort[ipos] * Intgwt[indx];
                            }
                        }
                        col_count++;
                    }
                    ...
                    for (m = -l; m <= l; m++) {
                        ...
                        if (pSPARC->CyclixFlag) {
                            for (ipos = 0; ipos < ndc; ipos++) {
                                ...
                                // indx = Atom_Influence_nloc[ityp].grid_pos[iat][Ind_sort[ipos]];
                                // (*nlocProj)[ityp].Chi_c_cyclix[iat][col_count*ndc+Ind_sort[ipos]] = Ylm[Ind_sort[ipos]] * UdV_sort[ipos] * Intgwt[indx];
                            }
                        }
                        col_count++;
                    }
