...
void Calculate_Vxc(SPARC_OBJ *pSPARC)
{
    double *rho;
    int sz_rho, i;
    sz_rho = pSPARC->Nd_d * (pSPARC->Nspden/2*2+1);
    ...
    if(strcmpi(pSPARC->XC,"LDA_PW") == 0 || strcmpi(pSPARC->XC,"LDA_PZ") == 0)
        ...
        || strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0 || strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0)
...
void Calculate_Vxc_GGA(SPARC_OBJ *pSPARC, double *rho)
{
    ...
    if (pSPARC->spin_typ == 0) { // spin-paired calculation
        ...
        else if(strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0) {
            // Perdew-Burke Ernzerhof exchange-correlation where exchange part is a numerical operation by convolution
            Calculate_Vxc_GGA_CONV_PBE(pSPARC, &xc_cst, rho);
        }
        else if (strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0) {
            // Similar to PBE_alpha where alpha is a function of multipole descriptors
            Calculate_Vxc_GGA_CONV_PBE_MULTIPOLE(pSPARC, &xc_cst, rho);
        }
    ...
    } else if (pSPARC->spin_typ == 1) { // collinear spin
        if(strcmpi(pSPARC->XC,"GGA_PBE") == 0 || strcmpi(pSPARC->XC,"GGA_RPBE") == 0 || strcmpi(pSPARC->XC,"GGA_PBEsol") == 0
            || strcmpi(pSPARC->XC,"PBE0") == 0 || strcmpi(pSPARC->XC,"HF") == 0 || strcmpi(pSPARC->XC,"HSE") == 0)
            // Perdew-Burke Ernzerhof exchange-correlation 
            Calculate_Vxc_GSGA_PBE(pSPARC, &xc_cst, rho);
        else if (strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0){
            Calculate_Vxc_GSGA_CONV_PBE(pSPARC, &xc_cst, rho);
        }
        else if (strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0) {
            Calculate_Vxc_GSGA_CONV_PBE_MULTIPOLE(pSPARC, &xc_cst, rho);
        }
    ...
    } else if (pSPARC->spin_typ == 2) { // non-collinear spin
        assert(pSPARC->Nspden <= 2 && pSPARC->spin_typ <= 1);
    }
}
...
void Calculate_Vxc_GSGA_PBE(SPARC_OBJ *pSPARC, XCCST_OBJ *xc_cst, double *rho) {
    ...
    for(i = 0; i < DMnd; i++) {
        ...
        for(spn_i = 0; spn_i < 2; spn_i++){
            ...
            dssdg = 2.0 * coeffss;
            dfxdg = dfxdss * dssdg;
            pSPARC->Dxcdgrho[DMnd + spn_i*DMnd + i] = ex_lsd * rho_updn * dfxdg; // spin up and spin down
            exc += ex_gga * rho_updn;
            // Hybrid functional 
            if ((pSPARC->usefock > 0) && (pSPARC->usefock % 2 == 0) && strcmpi(pSPARC->XC,"PBE0") == 0) {
        ...
        // density, divided by the gradient of the density
        // (The v3.3 definition includes the division by the norm of the gradient)
        pSPARC->Dxcdgrho[i] = (rho[i] * dtt_dg * dhh_dtt);// correlation part
    }
    ...
    for(i = 0; i < DMnd; i++){
        pSPARC->XCPotential[i] += -DDrho_x[i] - DDrho_y[i] - DDrho_z[i] - DDrho_x[DMnd + i] - DDrho_y[DMnd + i] - DDrho_z[DMnd + i];// correlation and spin up exchange potential
        pSPARC->XCPotential[DMnd + i] += -DDrho_x[i] - DDrho_y[i] - DDrho_z[i] - DDrho_x[2*DMnd + i] - DDrho_y[2*DMnd + i] - DDrho_z[2*DMnd + i];// correlation and spin down exchange potential
    }  
...
void Calculate_Exc(SPARC_OBJ *pSPARC, double *electronDens)
{
    ...
    if(strcmpi(pSPARC->XC,"LDA_PW") == 0 || strcmpi(pSPARC->XC,"LDA_PZ") == 0)
        ...
        || strcmpi(pSPARC->XC,"PBE0") == 0 || strcmpi(pSPARC->XC,"HF") == 0 || strcmpi(pSPARC->XC,"HSE") == 0
        || strcmpi(pSPARC->XC,"vdWDF1") == 0 || strcmpi(pSPARC->XC,"vdWDF2") == 0 || strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0  || strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0)
...
{
    if(pSPARC->spin_typ == 0) { // spin unpolarized
        if(strcmpi(pSPARC->XC,"GGA_PBE") == 0 || strcmpi(pSPARC->XC,"GGA_RPBE") == 0 || strcmpi(pSPARC->XC,"GGA_PBEsol") == 0 || strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0
            || strcmpi(pSPARC->XC,"PBE0") == 0 || strcmpi(pSPARC->XC,"HF") == 0 || strcmpi(pSPARC->XC,"HSE") == 0 || strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0) {
            // Perdew-Burke-Ernzerhof exchange correlation 
        ...
        if(strcmpi(pSPARC->XC,"GGA_PBE") == 0 || strcmpi(pSPARC->XC,"GGA_RPBE") == 0 || strcmpi(pSPARC->XC,"GGA_PBEsol") == 0
            || strcmpi(pSPARC->XC,"PBE0") == 0 || strcmpi(pSPARC->XC,"HF") == 0 || strcmpi(pSPARC->XC,"HSE") == 0 || strcmpi(pSPARC->XC, "GGA_CONV_PBE") == 0
            || strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0) {
            // Perdew-Burke-Ernzerhof exchange correlation 
