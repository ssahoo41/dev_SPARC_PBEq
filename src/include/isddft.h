...
typedef struct _SPARC_OBJ{
    ...
    // double *scfElectronDens;      // SCF electron density (for LPMP)- remove it later after adding the gather function
    ...
    int CalcMCSHFlag; // can be 1 (at the end of SCF), 2 (after every SCF)
    int MCSHRadialFunctionType; // HSMP or LPMP
    int MCSHRadialFunctionMaxOrder; // LPMP order
    int MCSHMaxMCSHOrder; // Maximum order of spherical harmonics
    double MCSHMaxRCutoff; 
    double MCSHRStepSize;
    // the next two values are for GGA_CONV_PBE_MULTIPOLE
    double m_val;
    double n_val;
    ...
    double *ExxRho_approx;      // approximate Exact exchange energy density
...
typedef struct _SPARC_INPUT_OBJ{ 
    ...
    double MCSHMaxRCutoff;
    double MCSHRStepSize;
    double m_val; // the next two values are for GGA_CONV_PBE_MULTIPOLE
    double n_val;
