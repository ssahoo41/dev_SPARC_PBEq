...
void read_input(SPARC_INPUT_OBJ *pSPARC_Input, SPARC_OBJ *pSPARC) {
    ...
    while (!feof(input_fp)) {
        ...
        /* OFDFT input options */
           ...
           fscanf(input_fp, "%*[^\n]\n");
            ...
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str, "N_VAL:") == 0) {
            fscanf(input_fp, "%lf", &pSPARC_Input->n_val);
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str, "M_VAL:") == 0) {
            fscanf(input_fp, "%lf", &pSPARC_Input->m_val);
            fscanf(input_fp, "%*[^\n]\n");
        } else {
    ...
    // check if GGA_CONV_PBE_MULTIPOLE and RELAX_FLAG > 0 are specified simultaneously
    if (strcmpi(pSPARC_Input->XC, "GGA_CONV_PBE_MULTIPOLE") == 0 && pSPARC_Input->RelaxFlag != 0) {
        printf("\nThis is an experimental functional. Structural relaxation is not implemented for convolutional XC functional!\n");
        exit(EXIT_FAILURE); 
    }

    // check if GGA_CONV_PBE_MULTIPOLE and CALC_STRESS are specified simultaneously
    if (strcmpi(pSPARC_Input->XC, "GGA_CONV_PBE_MULTIPOLE") == 0 && pSPARC_Input->Calc_stress == 1) {
        printf("\nThis is an experimental functional. Stress calculation is not implemented for convolutional XC functional!\n");
        exit(EXIT_FAILURE); 
    }
    // check if GGA_CONV_PBE_MULTIPOLE and CALC_PRES are specified simultaneously
    if (strcmpi(pSPARC_Input->XC, "GGA_CONV_PBE_MULTIPOLE") == 0 && pSPARC_Input->Calc_pres == 1) {
        printf("\nThis is an experimental functional. Pressure calculation is not implemented for convolutional XC functional!\n");
        exit(EXIT_FAILURE); 
    }
    // check if GGA_CONV_PBE_MULTIPOLE and MD_FLAG are specified simultaneously
    if (strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0 && pSPARC_Input->MDFlag == 1) {
        printf("\nThis is an experimental functional. Molecular dynamics is not implemented for convolutional XC functional!\n");
        exit(EXIT_FAILURE); 
    }
