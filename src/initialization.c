...
#define N_MEMBR 199 // 8 inputs for MCSH part
...
void set_defaults(SPARC_INPUT_OBJ *pSPARC_Input, SPARC_OBJ *pSPARC) {
    ...
    /* Default parameters for multipole features calculations */
    ...
    pSPARC_Input->MCSHMaxRCutoff = 3.0;
    pSPARC_Input->MCSHRStepSize = 3.0;
    pSPARC_Input->m_val = 0.0;
    pSPARC_Input->n_val = 0.0;
...
void SPARC_copy_input(SPARC_OBJ *pSPARC, SPARC_INPUT_OBJ *pSPARC_Input) {
    ...
    // GGA_CONV_PBE_MULTIPOLE value
    pSPARC->m_val = pSPARC_Input->m_val;
    if(!rank) printf("M_val = %f", pSPARC->m_val);
    pSPARC->n_val = pSPARC_Input->n_val;
    if(!rank) printf("N_val = %f", pSPARC->n_val);
    ...
    // check XC compatibility with pseudopotential, add option for GGA_PBE_CONV
    ...
    if (pSPARC->SQFlag == 1) {
        ...
        if (pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->SOC_Flag || pSPARC->usefock || pSPARC->mGGAflag) {
            if (!rank) 
    ...
    if (pSPARC->OFDFTFlag == 1) {
        ...
        if (pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->usefock || pSPARC->mGGAflag || pSPARC->SOC_Flag) {
            if (!rank) 
    ...
    if (pSPARC->SOC_Flag == 1) {
        if (pSPARC->usefock || pSPARC->SQFlag || pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->OFDFTFlag || pSPARC->mGGAflag) {
            if (rank == 0) 
    ...
    if (pSPARC->CyclixFlag == 1) {
        if (pSPARC->usefock || pSPARC->SQFlag || pSPARC->SQ3Flag || 
            pSPARC->CS_Flag || pSPARC->OFDFTFlag || pSPARC->mGGAflag || pSPARC->SOC_Flag) {
            if (rank == 0) 
                printf(RED "ERROR: Hybrid functional, SQ, SQ3, CS, OFDFT, SCAN and SOC are not supported in this version of Cyclix implementation.\n" RESET);
            exit(EXIT_FAILURE);
        }
        if (pSPARC->spin_typ == 1 && 
            (strcmpi(pSPARC->XC, "GGA_PBE") != 0) && (strcmpi(pSPARC->XC, "GGA_PBEsol") != 0) && (strcmpi(pSPARC->XC, "GGA_RPBE") != 0)) {
            if (rank == 0) 
                printf(RED "ERROR: GGA_PBE, GGA_PBEsol and GGA_RPBE with spin-polarization are not supported in this version of Cyclix implementation.\n" RESET);
            exit(EXIT_FAILURE);
        }
    }
...
void write_output_init(SPARC_OBJ *pSPARC) {
    ...
    // adding GGA_CONV_MULTIPOLE parameters to output file
    if (strcmpi(pSPARC->XC, "GGA_CONV_PBE_MULTIPOLE") == 0){
        fprintf(output_fp, "M_VAL: %f\n", pSPARC->m_val);
        fprintf(output_fp, "N_VAL: %f\n", pSPARC->n_val);
    }
    if (pSPARC->SQFlag == 1) {
...
void SPARC_Input_MPI_create(MPI_Datatype *pSPARC_INPUT_MPI) {
    ...
    MPI_Datatype SPARC_types[N_MEMBR] = {MPI_INT, MPI_INT, MPI_INT,                       /* int array */
                                         ...
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, 
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, /*double */ // added 2 for MCSH and 2 for m_val and n_val
    ...
    MPI_Get_address(&sparc_input_tmp.m_val, addr + i++);
    MPI_Get_address(&sparc_input_tmp.n_val, addr + i++);
