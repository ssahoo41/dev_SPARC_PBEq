...
void Init_electronDensity(SPARC_OBJ *pSPARC) {
    ...
    // double *global_init_rho_up, *global_init_rho_dn;
    // global_init_rho_up = (double *) malloc(pSPARC->Nd * sizeof(double));
    // global_init_rho_dn = (double *) malloc(pSPARC->Nd * sizeof(double));
    // int gridsizes[3] = {pSPARC->Nx, pSPARC->Ny, pSPARC->Nz};
    // gather_distributed_vector(pSPARC->electronDens + pSPARC->Nd_d, pSPARC->DMVertices, global_init_rho_up, gridsizes, pSPARC->dmcomm_phi, 1);
    // gather_distributed_vector(pSPARC->electronDens + 2 * pSPARC->Nd_d, pSPARC->DMVertices, global_init_rho_dn, gridsizes, pSPARC->dmcomm_phi, 1);

    // MPI_Bcast(global_init_rho_up, pSPARC->Nd, MPI_DOUBLE,  0, MPI_COMM_WORLD);
    // MPI_Bcast(global_init_rho_dn, pSPARC->Nd, MPI_DOUBLE,  0, MPI_COMM_WORLD);

    // if (rank == 0){
    //     char rhoUpFilename[128], rhoDnFilename[128];
    //     snprintf(rhoUpFilename, 128, "rho_init_up.csv");
    //     writeMatToFile(rhoUpFilename, global_init_rho_up, pSPARC->Nx, pSPARC->Ny, pSPARC->Nz);
    //     snprintf(rhoDnFilename, 128, "rho_init_dn.csv");
    //     writeMatToFile(rhoDnFilename, global_init_rho_dn, pSPARC->Nx, pSPARC->Ny, pSPARC->Nz);
    // }
}
