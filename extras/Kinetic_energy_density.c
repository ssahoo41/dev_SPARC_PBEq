 ...
 *          Including gradient way. Could be applied to Kinetic Energy.
 */
void computeKineticEnergyDensity(SPARC_OBJ *pSPARC)
{
    int rank;
    ...
    pSPARC->Krho = (double *) calloc(DMnd * (2*pSPARC->Nspin-1), sizeof(double));
    assert(pSPARC->Krho != NULL);
    Krho = pSPARC->Krho;
    memset(Krho, 0, sizeof(double) * DMnd * (2*pSPARC->Nspin-1));

    // flags for laplacian (1) or gradient (0)
    int lapOrGrad = 1;

    if (pSPARC->isGammaPoint == 1) {
        size_s = DMnd * Nband;
        if (lapOrGrad == 1) {
            LapX = (double *) calloc(DMnd * Nband, sizeof(double));  
            for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
                sg  = pSPARC->spin_start_indx + spn_i;
                X = pSPARC->Xorb + spn_i*size_s;
                Lap_vec_mult(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, Nband, 0.0, X, LapX, pSPARC->dmcomm);
                ...
                for (n = nstart; n <= nend; n++) {
                    ...
                    for (i = 0; i < DMnd; i++, count++) {
                        ...
                        Krho[sg*DMnd + i] -= g_nk * X[count] * LapX[count];
                    }
                }
            }
            free(LapX);
        ...
        size_s = size_k * Nk;
        if (lapOrGrad == 1) {
            LapX_kpt = (double _Complex *) calloc(DMnd * Nband, sizeof(double _Complex));

            for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
                sg  = pSPARC->spin_start_indx + spn_i;
                for (kpt = 0; kpt < Nk; kpt++) {
                    X_kpt = pSPARC->Xorb_kpt + kpt*size_k + spn_i*size_s;
                    Lap_vec_mult_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, Nband, 0.0, X_kpt, LapX_kpt, kpt, pSPARC->dmcomm);
                    ...
                    for (n = nstart; n <= nend; n++) {
                        ...
                        for (i = 0; i < DMnd; i++, count++) {
                            ...
                            // only total in case of non-spin-polarized calculation
                            // different from electron density 
                            Krho[sg*DMnd + i] -= g_nk * creal(conj(X_kpt[count]) * LapX_kpt[count]);
                        }
                    }
                }
            }
            ...
            for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
                ...
                for (kpt = 0; kpt < Nk; kpt++) {
                    X_kpt = pSPARC->Xorb_kpt + kpt*size_k + spn_i*size_s;
                    Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, Nband, 0.0, X_kpt, Dx_kpt, 0, kpt, pSPARC->dmcomm);
                    Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, Nband, 0.0, X_kpt, Dy_kpt, 1, kpt, pSPARC->dmcomm);
                    Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, Nband, 0.0, X_kpt, Dz_kpt, 2, kpt, pSPARC->dmcomm);
    ...
    if (pSPARC->npkpt > 1 && pSPARC->spincomm_index == 0) {    
        ...
        t2 = MPI_Wtime();
    #ifdef DEBUG
        if (rank == 0) printf("rank = %d, --- Calculate kinetic energy density: reduce over all kpoint groups took %.3f ms\n", rank, (t2-t1)*1e3);
    #endif
    }
    ...
    if (pSPARC->npband > 1 && pSPARC->spincomm_index == 0 && pSPARC->kptcomm_index == 0) {
        ...
        t2 = MPI_Wtime();
    #ifdef DEBUG
        if (rank == 0) printf("rank = %d, --- Calculate kinetic energy density: reduce over all band groups took %.3f ms\n", rank, (t2-t1)*1e3);
    #endif
    }
    ...
    KE *= pSPARC->dV;
    if (!rank) printf("Kinetic Energy: %f\n", KE);
#endif
}