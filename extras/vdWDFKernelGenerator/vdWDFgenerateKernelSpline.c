...
void vdWDF_generate_kernel(char *kernelFileName, char *splineFileName) { // let the function unrelated to pSPARC
    ...
    print_kernel(kernelFileName, vdWDFkernelPhi, vdWDFd2Phidk2, nrpoints, nqs);
    ...
    print_d2ydx2(splineFileName, nqs, vdWDFd2Splineydx2);
...
void print_kernel(char* outputName, double **vdWDFkernelPhi, double ** vdWDFd2Phidk2, int nrpoints, int nqs) {
    ...
    fprintf(outputFile, "double **kernelPhi = pSPARC->vdWDFkernelPhi;\n");
    fprintf(outputFile, "double **d2Phidk2 = pSPARC->vdWDFd2Phidk2;\n");
    int q1, q2, index, kernelLabel;
    for (q1 = 0; q1 < nqs; q1++) {
        for (q2 = 0; q2 <= q1; q2++) {
            // fprintf(outputFile, "first q %d, second q %d\n", q1, q2);
            kernelLabel = kernel_label(q1, q2, 20);
            fprintf(outputFile, "double theKernel%d[1025] = {", kernelLabel);
            // fprintf(outputFile, "Kernel function\n");
            for (index = 0; index < nrpoints; index++) {
                fprintf(outputFile, "%12.9f,\n", vdWDFkernelPhi[kernelLabel][index]);
            }
            fprintf(outputFile, "%12.9f};\n", vdWDFkernelPhi[kernelLabel][nrpoints]);
            fprintf(outputFile, "memcpy(kernelPhi[%d], theKernel%d, sizeof(double)*1025);\n", kernelLabel, kernelLabel);
            // fprintf(outputFile, "2nd derivative of Kernel function\n");
            fprintf(outputFile, "double thed2Phidk2%d[1025] = {", kernelLabel);
            for (index = 0; index < nrpoints; index++) {
                fprintf(outputFile, "%12.9f,\n", vdWDFd2Phidk2[kernelLabel][index]);
            }
            fprintf(outputFile, "%12.9f};\n", vdWDFd2Phidk2[kernelLabel][nrpoints]);
            fprintf(outputFile, "memcpy(d2Phidk2[%d], thed2Phidk2%d, sizeof(double)*1025);\n", kernelLabel, kernelLabel);
        }
    }
    fclose(outputFile);
}
...
void print_d2ydx2(char* outputName2, int nqs, double **vdWDFd2Splineydx2) {
    ...
    outputFile2 = fopen(outputName2,"a");
    fprintf(outputFile2, "double **d2Splineydx2 = pSPARC->vdWDFd2Splineydx2;\n");
    // fprintf(outputFile2, "dy2_dx2 of spline function\n");
    for (int q1 = 0; q1 < nqs; q1++) {
        for (int q2 = 0; q2 < nqs; q2++) {
            fprintf(outputFile2, "d2Splineydx2[%d][%d]=%12.9f; ", q1, q2, vdWDFd2Splineydx2[q1][q2]);
        }
        fprintf(outputFile2, "\n");
    }
    fclose(outputFile2);
